...
func TestFunctionObject(t *testing.T) {
	...
	if fn.Body.String() != "(x + 2)" {
		t.Fatalf("Body of the function is wrong, expected='(x+2)', got=%q",fn.Body.String())
	}
}
...
func TestStringLiteral(t *testing.T) {
	...
	if !ok {
		t.Fatalf("evaluated is not Stirng, got=%T(%v)",evaluated,evaluated)
	}

	if str.Value != "Hello World!" {
		t.Errorf("Wrong string value, expected=\"Hello World!\", got=%q",str.Value)
	}
}
...
func TestStringConcatenation(t *testing.T) {
	...
	if str.Value != "Hello World" {
		t.Fatalf("str.Value is wrong, got=%q",str.Value)
	}
}
	...
	for _,tt := range tests {
		...
		case string:
			...
			if errObj.Message != expected {
				t.Fatalf("wrong error message = %q, expected=%q",errObj.Message,expected)
			}
		}
	}
